...
@Component({
  selector: 'app-fees-grade-pending',
  imports: [CommonModule,
    MatTableModule,
    MatButtonModule,MatCardModule],
  templateUrl: './fees-grade-pending.component.html',
  styleUrl: './fees-grade-pending.component.css'
})
export class FeesGradePendingComponent implements OnInit {
  students : any[] =[];
  displayedColumns: string[] = ['full_name', 'father_name', 'contact_number', 'view_details'];
  gradeId:number=0;
  date : string='';
  ...
  getStudents():void{
    this.feeService.getPendingFeeStudents(this.gradeId,this.date).subscribe({
      next: (data) => {
        this.students = data;
